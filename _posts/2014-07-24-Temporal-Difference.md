...
After finishing up the original simulator, we started to work on extending it, and the underlying model. One of the features we'd included in the original was the ability to use configural cues, which were a later addition to the Rescorla-Wagner model. To briefly explain what that actually means - imagine that we are Pavlov, training our dogs to dribble at the bing of a bell. (Visualise stroking your [impressive beard](http://upload.wikimedia.org/wikipedia/commons/7/7d/Ivan_Pavlov_NLM3.jpg), if that helps.)  Now, as well as the bell, we want to train the dogs to dribble when they see a light, and to not dribble if both of those happen at once.

Now, I know you're an experienced dribble wrangler, but let me sketch out how you achieve this. Clearly, you need to bing the bell, or flash a torch or somesuch when you feed the dog. This will associate the bell, and the light, with food and bring the drool. But, we only want the dog to slobber (let's assume we're not using [Beethoven](http://cdn.makeagif.com/media/5-26-2014/AiKytX.gif) for our experiment), when one happens alone and not if they happen together. So, we also need to perform the flash-bing manoeuvre every so often, and not feed the dog.
...
A and B pretty much cancel each other out[^1], which leaves us with A-then-B, which as you'd hope gives you a dog expecting his dinner, and B-then-A, which leads to this:
...
[^1]: Ok, so you don't get no association at all, but this was a quick demo and we could probably do better with tweaked parameters.